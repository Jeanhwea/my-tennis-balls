...
void VisualUtil::drawGradientBackground(Node *parent, const Size &size, const GradientConfig &config,
                                        int zOrder)
{
    ...
    for (int i = 0; i < config.strips; ++i) {
        ...
        Color4F c(config.botColor.r + (config.topColor.r - config.botColor.r) * (t0 + t1) / 2,
                  ...
                  config.botColor.b + (config.topColor.b - config.botColor.b) * (t0 + t1) / 2, 1.0f);
        bg->drawSolidRect(Vec2(0, size.height * t0), Vec2(size.width, size.height * t1), c);
    }
    parent->addChild(bg, zOrder);
}

void VisualUtil::drawGrid(Node *parent, const Size &size, const GridConfig &config, int zOrder)
{
...
void VisualUtil::drawCorners(Node *parent, const Size &size, const CornerConfig &config, int zOrder)
{
